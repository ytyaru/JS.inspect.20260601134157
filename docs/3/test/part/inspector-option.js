...
describe("InspectorOption", () => {
    ...
    describe("create()", () => {
        ...
        describe("例外発生", () => {
            ...
            test('({x:1})',()=>terr(()=>InspectorOption.create({x:1}), Error, `引数不正。xは不正なキーです。期待されるキー:[str, sym, ins]`));
            test('({str:{x:1}})',()=>terr(()=>InspectorOption.create({str:{x:1}}), Error, `引数不正。str.xは不正なキーです。期待されるキー:[quote]`));
            test('({sym:{x:1}})',()=>terr(()=>InspectorOption.create({sym:{x:1}}), Error, `引数不正。sym.xは不正なキーです。期待されるキー:[quote, mode]`));
            test('({ins:{x:1}})',()=>terr(()=>InspectorOption.create({ins:{x:1}}), Error, `引数不正。ins.xは不正なキーです。期待されるキー:[mode]`));
            test('({str:1})',()=>terr(()=>InspectorOption.create({str:1}), Error, `引数不正。strは{quote:"'"}またはv=>\`"\${v}"\`等であるべきです。`));
            test('({sym:1})',()=>terr(()=>InspectorOption.create({sym:1}), Error, `引数不正。symは{quote:"'",mode:"standard"}またはv=>\`Symbol("\${v.description}")\`等であるべきです。`));
            test('({ins:1})',()=>terr(()=>InspectorOption.create({ins:1}), Error, `引数不正。insは{mode:"lost"}またはv=>\`new \${v.constructor.name}(/*引数情報消失*/)"\`等であるべきです。`));
        ...
        describe("正常", () => {
            ...
            describe("規定プロパティに既定値をセットでき対象以外が変更されないこと", () => {
                ...
                test.each([['standard', 'exception', 'raw']])('sym.mode=%p',(v)=>{
                ...
                test.each([['lost', 'exception']])('ins.mode=%p',(v)=>{
